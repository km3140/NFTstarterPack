...
contract testHatjae is TokenPrice{
    ...
    // 티켓 구매 완료 이벤트
    event ticketSold(address _who, string[1] _order);
...
//---------------main function---------------
    ...
    function buyTicket (
        ...
        // 티켓 지급(?)
        ticketBox[round][msg.sender].push(Ticket(msg.sender, [_first], _said));

        // 현재 라운드 티켓 수 count
        ticketCount[round]++;

        // 구매자 주소와 구매자가 선택한 순위 emit
        emit ticketSold(msg.sender, [_first]);
    }
